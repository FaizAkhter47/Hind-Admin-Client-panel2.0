...
const SETTINGS_KEY = "hcs-global-admin-settings";
const defaultGlobalAdminSettings: GlobalAdminSettings = {
  account: { adminId: "HCS-ADMIN", username: "admin", fullName: "HCS Administrator", active: true },
  security: { minPasswordLength: 8, failedAttemptsLimit: 5, requireUppercase: true, requireNumber: true, requireSpecialCharacter: true },
  general: {},
  clients: [],
};
function getGlobalAdminSettings(): GlobalAdminSettings {
  if (typeof window === "undefined") return clone(defaultGlobalAdminSettings);
  try { return { ...clone(defaultGlobalAdminSettings), ...JSON.parse(localStorage.getItem(SETTINGS_KEY) || "{}") }; } catch { return clone(defaultGlobalAdminSettings); }
}
function saveGlobalAdminSettings(value: GlobalAdminSettings) {
  localStorage.setItem(SETTINGS_KEY, JSON.stringify(value));
  window.dispatchEvent(new CustomEvent("hcs-admin-settings-updated"));
}
function getClientAccounts() { return getGlobalAdminSettings().clients; }
function saveClientAccount(client: ClientAccount) {
  const settings = getGlobalAdminSettings();
  saveGlobalAdminSettings({ ...settings, clients: settings.clients.some(x => x.id === client.id) ? settings.clients.map(x => x.id === client.id ? client : x) : [...settings.clients, client] });
}
function deleteClientAccount(id: string) {
  const settings = getGlobalAdminSettings();
  saveGlobalAdminSettings({ ...settings, clients: settings.clients.filter(x => x.id !== id) });
}
...
function makeInitials(v: string) { const p = v.trim().split(/\s+/).filter(Boolean).slice(0, 2); return p.length ? p.map(x => x[0].toUpperCase()).join("") : "HC"; }
...
export default function AdminPage() {
  ...
  function submitClient(e: FormEvent) {
    ...
    saveClientAccount(saved); setDrawer(null); load(); note(editingClient ? "Client updated." : "Client created.");
  ...
  function resetPassword(c: ExtendedClient) {
    const password = passwordFor(settings?.security ?? {}); const saved = { ...c, password, updatedAt: new Date().toISOString() }; saveClientAccount(saved);
  ...
  function togglePortal(c: ExtendedClient) { const enabled = c.clientPortalEnabled === false; saveClientAccount({ ...c, clientPortalEnabled: enabled, active: enabled && c.status === "Active", updatedAt: new Date().toISOString() }); load(); note(enabled ? "Portal enabled." : "Portal disabled."); }
  function setStatus(c: ExtendedClient, status: ClientStatus) { saveClientAccount({ ...c, status, active: status === "Active" && c.clientPortalEnabled !== false, updatedAt: new Date().toISOString() }); load(); note(`Client set to ${status}.`); }
  ...
  function bulk(action: "enable" | "disable" | "suspend") {
    const cur = getGlobalAdminSettings(); const ids = new Set(selected); const list = (cur.clients ?? []).map((c: ExtendedClient) => {
  ...
  function saveSettings() { if (!draftSettings) return; const e = validatePassword(draftSettings.account?.password ?? "", draftSettings.security); if (e) return note(e); saveGlobalAdminSettings(draftSettings); setSettings(clone(draftSettings)); note("Admin settings saved."); }
  ...
  return (
    <div className="hcs-admin-page">
      ...
      <main className="admin-main">
        ...
        <section className="admin-content">
          ...
          {section === "settings" && draftSettings && <section className="settings-content"><div className="settings-grid">
            <section className="panel"><div className="panel-heading"><div><span className="eyebrow">ADMIN ACCOUNT</span><h3>Administrator identity</h3></div></div><div className="form-grid"><Field label="Admin ID"><input value={draftSettings.account?.adminId ?? ""} onChange={e => setDraftSettings((v: GlobalAdminSettings | null) => v ? ({ ...v, account: { ...v.account!, adminId: e.target.value } }) : v)} /></Field><Field label="Username"><input value={draftSettings.account?.username ?? ""} onChange={e => setDraftSettings((v: GlobalAdminSettings | null) => v ? ({ ...v, account: { ...v.account!, username: e.target.value } }) : v)} /></Field><Field label="Full Name"><input value={draftSettings.account?.fullName ?? ""} onChange={e => setDraftSettings((v: GlobalAdminSettings | null) => v ? ({ ...v, account: { ...v.account!, fullName: e.target.value } }) : v)} /></Field><Field label="Email"><input value={draftSettings.account?.email ?? ""} onChange={e => setDraftSettings((v: GlobalAdminSettings | null) => v ? ({ ...v, account: { ...v.account!, email: e.target.value } }) : v)} /></Field><Field label="Phone"><input value={draftSettings.account?.phone ?? ""} onChange={e => setDraftSettings((v: GlobalAdminSettings | null) => v ? ({ ...v, account: { ...v.account!, phone: e.target.value } }) : v)} /></Field><Field label="Job Title"><input value={draftSettings.account?.jobTitle ?? ""} onChange={e => setDraftSettings((v: GlobalAdminSettings | null) => v ? ({ ...v, account: { ...v.account!, jobTitle: e.target.value } }) : v)} /></Field></div></section>
                        <section className="panel"><div className="panel-heading"><div><span className="eyebrow">SECURITY</span><h3>Admin password</h3></div></div><Field label="Password"><input type="password" value={draftSettings.account?.password ?? ""} onChange={e => setDraftSettings((v: GlobalAdminSettings | null) => v ? ({ ...v, account: { ...v.account!, password: e.target.value, loginPassword: e.target.value } }) : v)} /></Field><p className="field-help">Changing this password updates the unified Admin login.</p><label className="switch-row"><input type="checkbox" checked={draftSettings.account?.active !== false} onChange={e => setDraftSettings((v: GlobalAdminSettings | null) => v ? ({ ...v, account: { ...v.account!, active: e.target.checked } }) : v)} /><span>Admin account active</span></label></section>
            <section className="panel settings-span"><div className="panel-heading"><div><span className="eyebrow">CLIENT PASSWORD POLICY</span><h3>Security rules used by generated/reset client passwords</h3></div></div><div className="form-grid"><Field label="Minimum Length"><input type="number" min={6} value={draftSettings.security?.minPasswordLength ?? 8} onChange={e => setDraftSettings((v: GlobalAdminSettings | null) => v ? ({ ...v, security: { ...v.security, minPasswordLength: Math.max(6, Number(e.target.value) || 8) } }) : v)} /></Field><Field label="Failed Attempts Limit"><input type="number" min={1} value={draftSettings.security?.failedAttemptsLimit ?? 5} onChange={e => setDraftSettings((v: GlobalAdminSettings | null) => v ? ({ ...v, security: { ...v.security, failedAttemptsLimit: Math.max(1, Number(e.target.value) || 5) } }) : v)} /></Field></div><div className="switch-grid">{[["requireUppercase","Require uppercase"],["requireNumber","Require number"],["requireSpecialCharacter","Require special character"],["blockAfterFailedAttempts","Block after failed attempts"],["loginAlerts","Login alerts"],["suspiciousLoginAlerts","Suspicious login alerts"],["sessionAlerts","Session alerts"],["passwordExpiry","Password expiry"]].map(([key,label]) => <label className="switch-row" key={key}><input type="checkbox" checked={Boolean((draftSettings.security as AnyRecord)?.[key])} onChange={e => setDraftSettings((v: GlobalAdminSettings | null) => v ? ({ ...v, security: { ...v.security, [key]: e.target.checked } }) : v)} /><span>{label}</span></label>)}</div></section>
            <section className="panel settings-span"><div className="panel-heading"><div><span className="eyebrow">GENERAL</span><h3>Company information</h3></div></div><div className="form-grid"><Field label="Company Name"><input value={draftSettings.general?.companyName ?? ""} onChange={e => setDraftSettings((v: GlobalAdminSettings | null) => v ? ({ ...v, general: { ...v.general, companyName: e.target.value } }) : v)} /></Field><Field label="Company Email"><input value={draftSettings.general?.companyEmail ?? ""} onChange={e => setDraftSettings((v: GlobalAdminSettings | null) => v ? ({ ...v, general: { ...v.general, companyEmail: e.target.value } }) : v)} /></Field><Field label="Support Email"><input value={draftSettings.general?.supportEmail ?? ""} onChange={e => setDraftSettings((v: GlobalAdminSettings | null) => v ? ({ ...v, general: { ...v.general, supportEmail: e.target.value } }) : v)} /></Field><Field label="Phone"><input value={draftSettings.general?.phone ?? ""} onChange={e => setDraftSettings((v: GlobalAdminSettings | null) => v ? ({ ...v, general: { ...v.general, phone: e.target.value } }) : v)} /></Field><Field label="Website"><input value={draftSettings.general?.website ?? ""} onChange={e => setDraftSettings((v: GlobalAdminSettings | null) => v ? ({ ...v, general: { ...v.general, website: e.target.value } }) : v)} /></Field><Field label="Timezone"><input value={draftSettings.general?.timezone ?? ""} onChange={e => setDraftSettings((v: GlobalAdminSettings | null) => v ? ({ ...v, general: { ...v.general, timezone: e.target.value } }) : v)} /></Field></div></section>
      ...
      {drawer && <ClientDrawer form={clientForm} setForm={setClientForm} websites={websites} security={settings.security} mode={drawer} showPassword={showPassword} setShowPassword={setShowPassword} onClose={() => setDrawer(null)} onSubmit={submitClient} />}
...
function ClientDrawer({ form, setForm, websites, security, mode, showPassword, setShowPassword, onClose, onSubmit }: any) {
  const set = (k: keyof ClientForm, v: any) => setForm((p: ClientForm) => ({ ...p, [k]: v }));
  const toggle = (k: PermissionKey) => set("permissions", { ...(form.permissions ?? {}), [k]: !Boolean(form.permissions?.[k]) });
